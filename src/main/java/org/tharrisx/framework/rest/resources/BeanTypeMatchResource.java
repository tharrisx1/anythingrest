...
public abstract class BeanTypeMatchResource<T extends StorableBean> {
  ...
  public BeanTypeMatchResource(final ResourceCore<T> resourceCore1, final ResourceRequestInfo resourceRequestInfo1) {
    if(Log.isEnteringEnabled(BeanTypeMatchResource.class)) Log.entering(BeanTypeMatchResource.class, Log.METHOD_NAME_CONSTRUCTOR, resourceCore1, resourceRequestInfo1);
    this.resourceCore = resourceCore1;
    this.resourceRequestInfo = resourceRequestInfo1;
    if(Log.isExitingEnabled(BeanTypeMatchResource.class)) Log.exiting(BeanTypeMatchResource.class, Log.METHOD_NAME_CONSTRUCTOR);
  }
  ...
  public Response matchBeans(
    ...
    try {
      ...
      if(Log.isExitingEnabled(getClass())) Log.exiting(getClass(), METHOD_MATCH_BEANS, ret);
    }
  }
